...
; Esc::ExitApp() ; Removed per user request
...
ShowSafetyAlertMsg()
{
    ...
    SafetyAlertGui.BackColor := "080008"
    SafetyAlertGui.SetFont("s10 cFF4DFF Bold", "Segoe UI")
...
ShowOneHourPauseAlertMsg()
{
    ...
    OneHourPauseAlertGui.BackColor := "080008"
    OneHourPauseAlertGui.SetFont("s11 cFF4DFF Bold", "Segoe UI")
...
InitSessionWindow()
{
    ...
    SessionGui := Gui("+AlwaysOnTop +ToolWindow -Caption", "AANOReserve - No Reserve")
    SessionGui.BackColor := "080008"
    SessionGui.SetFont("s10 cFF00FF Bold", "Segoe UI")
    ...
    SessionToggleBtn := SessionGui.AddText("x12 y34 w260 h18 cFF4DFF Center", "Session: 0 [Live]")
    SessionMinusBtn := SessionGui.AddText("x14 y34 w18 h18 cFF00FF Center", "-")
    SessionPlusBtn := SessionGui.AddText("x252 y34 w18 h18 cFF00FF Center", "+")
    SessionText.SetFont("s11 cFF00FF Bold", "Segoe UI")
    SessionToggleBtn.SetFont("s9 cFF4DFF Bold", "Segoe UI")
    SessionMinusBtn.SetFont("s13 cFF00FF Bold", "Segoe UI")
    SessionPlusBtn.SetFont("s13 cFF00FF Bold", "Segoe UI")
    
    ; === BOT STATUS CONSOLE (Replaces Clock/F8 Steps) ===
    SessionClockText := SessionGui.AddText("x12 y68 w200 h32 cFF4DFF Center", "BOT: PAUSED")
    AutoClerkToggleBtn := SessionGui.AddText("x217 y66 w35 h28 cFF00FF Background000000 Border Center", "▶")
    AutoClerkToggleBtn.SetFont("s12 cFF00FF Bold", "Segoe UI Emoji")
    ...
    CalcInput := SessionGui.AddEdit("x122 y116 w150 h31 cFF4DFF Background140014 Center -E0x200")
    
    CalcBaseLabel := SessionGui.AddText("x12 y150 w110 h24", "Bid Estimate")
    CalcBaseText := SessionGui.AddText("x122 y152 w150 h31 cFF4DFF Background140014 Center", "0")
    
    CalcResultLabel := SessionGui.AddText("x12 y180 w110 h24", "Final")
    CalcFinalText := SessionGui.AddText("x122 y188 w150 h31 cFF4DFF Background140014 Center", "0")
    
    CalcInternetLabel := SessionGui.AddText("x12 y215 w110 h24", "Internet")
    CalcInternetText := SessionGui.AddText("x122 y222 w150 h31 cFFD700 Background140014 Center", "$0")
    ...
    CalcTimerText := SessionGui.AddText("x12 y240 w200 h22 cFF00FF", "BOT: PAUSED")
    CalcTimerText.SetFont("s10 Bold", "Segoe UI")
    
    ; Mute button
    MuteBtn := SessionGui.AddText("x220 y238 w50 h26 cFF00FF Background000000 Border Center", "🔊")
    MuteBtn.SetFont("s13 cFF00FF Bold", "Segoe UI Emoji")
    ...
    PacingLPHLabel := SessionGui.AddText("cFF00FF BackgroundTrans", "LPH:")
    PacingLPHLabel.SetFont("s10 cFF00FF Bold", "Segoe UI")
    tgtLPHMem := ""
    try tgtLPHMem := IniRead(A_ScriptDir "\AuctionSessions.ini", "Pacing", "TargetLPH", "")
    PacingLPHFrame := SessionGui.AddText("BackgroundFF00FF")
    PacingLPHInput := SessionGui.AddEdit("cFF4DFF Background140014 Center -E0x200", tgtLPHMem)
    PacingLPHInput.SetFont("s10 Bold", "Segoe UI")
    PacingNextLabel := SessionGui.AddText("cFF4DFF BackgroundTrans", "Calc")
    PacingNextLabel.SetFont("s10 Bold", "Segoe UI")
    AuctionUIDLinkBtn := SessionGui.AddText("cFF00FF Background000000 Border Center +0x200", "EOA")
    AuctionUIDLinkBtn.SetFont("s11 cFF00FF Bold", "Segoe UI")
    ...
    ; === ROW 8: Remap Button ===
    CalcRemapBtn := SessionGui.AddText("x12 y331 w260 h36 cFF00FF Background000000 Border Center Hidden", "Remap")
    ...
    ; Typography cleanup for consistent visual weight.
    CalcAmountLabel.SetFont("s10 cFF00FF Bold", "Segoe UI")
    CalcBaseLabel.SetFont("s10 cFF00FF Bold", "Segoe UI")
    CalcResultLabel.SetFont("s10 cFF00FF Bold", "Segoe UI")
    CalcInput.SetFont("s14 cFF4DFF Bold", "Segoe UI")
    CalcBaseText.SetFont("s14 cFF4DFF Bold", "Segoe UI")
    CalcFinalText.SetFont("s15 cFF4DFF Bold", "Segoe UI")
    CalcRemapBtn.SetFont("s10 cFF00FF Bold", "Segoe UI")
    CalcPercent1Radio.SetFont("s10 cFF00FF Bold", "Segoe UI")
    CalcPercent5Radio.SetFont("s10 cFF00FF Bold", "Segoe UI")
    CalcPercent10Radio.SetFont("s10 cFF00FF Bold", "Segoe UI")
    CalcSpamSlowRadio.SetFont("s10 cFF00FF Bold", "Segoe UI")
    CalcSpamMediumRadio.SetFont("s10 cFF00FF Bold", "Segoe UI")
    CalcSpamFastRadio.SetFont("s10 cFF00FF Bold", "Segoe UI")
    CalcWaitDropDown.SetFont("s10 cFF00FF Bold", "Segoe UI")
    
    PacingLotsLabel := SessionGui.AddText("cFF00FF BackgroundTrans", "Stop Lot:")
    tgtLotsMem := ""
    tgtHourMem := 6
    tgtMinMem  := 1
    tgtAmPmMem := 2
    ...
    PacingLotsFrame := SessionGui.AddText("BackgroundFF00FF")
    PacingHourFrame := SessionGui.AddText("BackgroundFF00FF")
    PacingMinFrame := SessionGui.AddText("BackgroundFF00FF")
    PacingLotsInput := SessionGui.AddEdit("cFF4DFF Background140014 Center Number -E0x200", tgtLotsMem)
    PacingMinsLabel := SessionGui.AddText("cFF00FF BackgroundTrans", "PH Time (GMT+8):")
    PacingHourDrop  := SessionGui.AddEdit("cFF4DFF Background140014 Center Number -E0x200", Clamp(Integer(tgtHourMem), 1, 12))
    PacingMinDrop   := SessionGui.AddEdit("cFF4DFF Background140014 Center Number -E0x200", Format("{:02}", Clamp(Integer(tgtMinMem) - 1, 0, 59)))
    amPmText := (Integer(tgtAmPmMem) = 2) ? "PM" : "AM"
    PacingAmPmDrop  := SessionGui.AddText("cFF4DFF Background000000 Border Center", amPmText)
    ...
    PacingToggleBtn := SessionGui.AddText("cFF00FF Background000000 Border Center", "BOT LOCK")
    PacingToggleBtn.SetFont("s11 cFF00FF Bold", "Segoe UI")
    PacingToggleBtn.OnEvent("Click", TogglePacing)
    PacingLotsLabel.SetFont("s9 cFF00FF Bold", "Segoe UI")
    PacingMinsLabel.SetFont("s9 cFF00FF Bold", "Segoe UI")
    
    PacingProgressText := SessionGui.AddText("cFF4DFF BackgroundTrans Center", "Lots: RUNNING")
    PacingProgressText.SetFont("s8 cFF4DFF", "Segoe UI")

    PacingHourlyRemainingText := SessionGui.AddText("cFF4DFF BackgroundTrans Left", "Lots: --")
    PacingHourlyRemainingText.SetFont("s8 cFF4DFF", "Segoe UI")
...
UpdateSessionWindow()
{
    ...
    if IsObject(SessionToggleBtn)
    {
        if (statusText = "TEMPORARY")
        {
            ...
            SessionToggleBtn.SetFont("cFF4DFF")
        }
        else
        {
            SessionToggleBtn.Text := "Session: " displaySession " [" statusText "]"
            SessionToggleBtn.SetFont("cFF00FF")
        }
    }
...
ApplyUILayout(minMax := 0)
{
    ...
    SessionMinusBtn.SetFont("s13 cFF00FF Bold")
    SessionToggleBtn.Move(32, 3, 126, 22)
    SessionToggleBtn.SetFont("s9 cFF4DFF Bold")
    SessionPlusBtn.Move(160, 3, 18, 22)
    SessionPlusBtn.SetFont("s13 cFF00FF Bold")
    ...
    CalcBaseLabel.SetFont("s7 cFF4DFF Bold")
    CalcInternetLabel.SetFont("s7 cFF4DFF Bold")

    CalcBaseText.Move(margin, 43, boxW, 38)
    CalcBaseText.SetFont("s15 cFF4DFF Bold")

    CalcFinalText.Move(off, off)

    CalcInternetText.Move(margin, 98, boxW, 38)
    CalcInternetText.SetFont("s15 cFF4DFF Bold")
    ...
    if (minMax != -1)
    {
        SessionGui.Show("w198 h294 NoActivate")
    }
}
...
UpdateExtensionCurrentPrice()
{
    ...
    global lastPersistentInternetPrice
    ...
    if (intRaw != "" && IsObject(CalcInternetText))
    {
        ...
        ; No-reserve safety does not compare Internet price to bid estimate.
        ; If a live Internet price exists while AutoClerk is active, reload.
        CalcInternetText.SetFont("cFFD700 Bold")
...
HandleF9()
{
    ...
    if (CurrentActiveLotID != "" && ExtCoordsCache.Has(CurrentActiveLotID))
    {
        ...
        if lotCache.Has("internet") && lotCache.Has("competing")
        {
            ...
            internetDeadline := A_TickCount + 100
            while (A_TickCount <= internetDeadline)
            {
            ...
            ; Only click Competing if we actually took an internet bid
            if (internetClicked)
            {
                Sleep(GetSpamDelay(50))  ; short settle so page registers internet first
...
HandleF10()
{
    ...
    global pacingStopPending, pacingTargetAbsoluteLotMark, currentAuctionProgressNumber
    ...
    ; 500ms delay before the final Fair/Going button.
    Sleep(500)
    
    ; Going/Fair AFTER spam, matching AAMICRO's F10 close.
    if (pressCount >= 11 && !pauseCountSession)
    {
        ClickFairSmart()
    ...
    ; Designated lot rule: exact target pauses after F10 instead of reloading.
    if HandleDesignatedLotStopGuard("f10")
        return
...
AlignAskToBidEstimateBeforeGoingFair()
{
    ...
    ; Read price with high reliability
...
HandleF11(isAuto := false)
{
    ...
    global allowF11, pauseCountSession, scriptPaused, checkButtonLatched, f11Paused, f10F11PressLock, autoClerkPhase
    ...
    ; --- NEXT SAFETY GATE ---
    ; No-reserve mode does not compare Internet price to bid estimate.
    ; If a live Internet price exists, do NOT click Next.
...
AutoClerkTick()
{
    ...
    if (autoClerkPhase = "WAITING")
    {
        ...
        ; If Internet Bid pops up during wait, KILL automation for safety
        ; --- PRICE-DRIVEN INTERRUPT (More reliable than simple flag) ---
        ...
        ; --- 59th MINUTE REST FEATURE ---
        phTime := GetPHTime()
        currentMin := SubStr(phTime, 11, 2)
        if (currentMin == "59")
        {
            RecalibratePacing()
            if (pacingCurrentHourLeft <= 0 && lotIsClosed)
            {
        ...
        ; --- INTERNET SAFETY DURING WAIT ---
        remainingWaitMs := autoClerkWaitMs - (A_TickCount - f10TriggerTime)
        global internetExtendFired
        
        ; In no-reserve mode, any live Internet price is a safety reload.
        if (remainingWaitMs < 30000 && !internetExtendFired)
        {
            if CheckInternetLimitReload()
            {
    ...
    if (autoClerkPhase = "NEXT")
    {
        ...
        if (CurrentActiveLotID != "" && lastAutoClerkLotToken != "" && CurrentActiveLotID != lastAutoClerkLotToken)
        {
            lastAutoClerkLotNumber := currentAuctionProgressNumber
            lastAutoClerkLotToken := CurrentActiveLotID
            global currentLotDidBid, internetExtendFired, timerSoundFired, countdownSeconds, timerActive
            ...
            ; --- NEW LOT SETTLE ---
            ; No-reserve mode starts with Last Call inside the F10 rhythm, so
            ; do not pre-click Internet/Competing when the lot changes.
            Sleep(200)

            if (GetLiveInternetSafetyNumber() > 0)
            {
                if IsObject(CalcTimerText)
                    CalcTimerText.Text := "SAFETY: INT BID ON NEW LOT"
                if IsObject(SessionClockText)
                    SessionClockText.Text := "BOT: SAFETY RELOAD"
    ...
    ; NO-RESERVE STARTUP: no F8/F9 ladder. Let prices settle once, then run
    ; the F10 close rhythm with the same internet/bid safety checks.
    ...
    ; NO-RESERVE: trigger the closer directly. No F8/F9 bid ladder.
    if (autoClerkPhase != "WAITING")
    {
        if (!WasEventDone("F10"))
        {
...
SharedSpam(mode := "EQUALS")
{
    ...
    ; No-reserve sequence:
    ; 1x Last Call -> Internet + Competing -> 3x registered Last Call.
    ; The page sometimes drops one rapid Last Call, so send 4 attempts here.
...
CheckSharedSpamDangerBeep()
{
    ...
    ; No-reserve mode reloads on any live Internet price.
...
ClickInternetThenCompeting(waitMs := 0, pollMs := 0, requireInternetReady := false, limitNum := 0)
{
    ...
    ; --- BRUTE FORCE WITH LIMIT GUARD ---
    
    ; Click 1: Internet (Always fire first)
    ClickPrimaryButtonNative("internet")
    ...
    ; Reset seen time if button is currently visible
    if (NormalizeToNumber(ReadInternetBidPriceFromExtension()) > 0 || NormalizeToNumber(ReadInternetBidPrice()) > 0)
        lastInternetSeenTime := A_TickCount
    
    ; Also ensure the lot-start counts as a "seen" event for the initial buffer
...
ClickLastCallSmart()
{
    return ClickPrimaryButtonNativeRetry("lastcall", 3, 120)
}
...
ClickFairSmart()
{
    return ClickPrimaryButtonNative("fair")
}
...
ForceUpdateCoordsCache()
{
    ...
    if IsObject(CalcTimerText)
    {
        ...
        CalcTimerText.SetFont("cFF00FF Bold")
        SetTimer(ResetF9TimerText, 1500)
    }
}
...
ExtractAuctionProgressLotNumber(progressText, activeLotId := "")
{
    ...
    ; "Lot 12 of 688" is an auction position counter. Pacing counts down from
    ; remaining lots, so the active lot value is 688 - 12 + 1.
...
PollActiveLotState()
{
    ...
    try
    {
        ...
        if (req.Status = 200)
        {
            ...
            ; --- DESIGNATED LOT GUARD ---
            global autoClerkActive, pacingTargetAbsoluteLotMark
            if HandleDesignatedLotStopGuard("poll")
                return
            ...
            if RegExMatch(body, '"activeLotId"\s*:\s*"([^"]+)"', &match)
            {
                newLotId := match[1]
                ...
                if (newLotId != CurrentActiveLotID)
                {
                    ...
                    {
                        if HandleDesignatedLotStopGuard("lot_change_pending")
                            return
                    }
                    ...
                    global lastPersistentInternetPrice, internetInterruptFired, internetBidPriceFile
                    lastPersistentInternetPrice := "0"
                    internetInterruptFired := false
                    if IsObject(CalcInternetText)
                        CalcInternetText.Text := "$0"

                    ; --- COORDS CACHE INVALIDATION ON LOT CHANGE ---
                    ; Clear stale coords for the old lot so the bot never clicks
                    ; wrong positions while the new lot's buttons are rendering.
                    ExtCoordsCache.Clear()
                    ; Give the extension 400ms to post fresh coords, then refresh cache.
                    SetTimer(ForceUpdateCoordsCache, -400)
        ...
        ; Trigger on 'Enabled' (Yellow) state of the DOM button for sessions 11-14.
        ; Using a LATCH to ensure it only happens ONCE per bid.
        if (pressCount >= 11 && !pauseCountSession)
        {
            if IsInternetReadyFromExtension(false)
            {
...
ClickPrimaryButtonNative(buttonName)
{
    ...
    ; Click WITHOUT focusing the window - this preserves your active window
...
ForceReloadDesignatedLotStop()
{
    ...
    ; Multi-hour/session pacing: reaching the hourly designated lot means
    ; pause and resume, not final-stop.
...
{
    ; A live Internet price is a real no-reserve safety stop, not an F4 stuck-bot resume.
    ForceReloadSafetyStop()
}
...
HandleDesignatedLotStopGuard(source := "")
{
    ...
    if (!autoClerkActive || pacingTargetAbsoluteLotMark <= 0 || currentAuctionProgressNumber <= 0)
        return false

    ; If telemetry jumped past the target, use the same multi-hour reload
    ; behavior as AAMICRO.
    ...
    ; On the designated lot, wait until F10 has completed, then stop/reload.
...
BuildProgressiveF8Ladder(target)
{
    ...
    else if (target <= 2000) ; 1001-2000 Bracket — key jumps, F9 fills the rest
    {
        milestones := [300, 500, 700, 1000, 1300]
    }
    else if (target < 10000) ; 4 Digits - Every $500
    {
    ...
    {
        milestones := [100, 500, 1000, 3000, 5000, 7000, 10000, 30000, 50000, 70000]
    }
    else ; 6 Digits+ - same base pattern then $10,000 steps
    {
...
IsInternetPriceActiveOrRecent(recentMs := 2000)
{
    ...
    if (intBidNum > 0 && IsInternetReadyFromExtension(true))
    {
        lastInternetSeenTime := A_TickCount
        return true
    }
...
ForceReloadIfInternetAtBidEstimate()
{
    return CheckInternetLimitReload()
}
...
CheckInternetLimitReload()
{
    if (IsInternetReadyFromExtension(true) && IsInternetAtBidEstimate(&bidEstNum, &intBidNum))
    {
        ForceReloadByReason("internet_limit")
        return true
    }
...
BidAmount(amount)
{
    ...
    ; WAVE 1: Accepts any immediate internet bid while we are preparing our price
    ClickPrimaryButtonNative("internet")
...
StopPacing(silent := false)
{
    ...
    if IsObject(PacingToggleBtn)
    {
        ...
        PacingToggleBtn.SetFont("cFF00FF") ; Magenta
    }
    
        if IsObject(PacingProgressText)
        {
            PacingProgressText.Text := "STOPPED"
            PacingProgressText.SetFont("cFF00FF")
        }
...
ApplyPacingCalculation(silent := false)
{
    ...
    if (DateDiff(enteredDeadline, A_Now, "Seconds") < -60)
    {
        pacingAbsoluteDeadline := ""
        pacingTargetAbsoluteLotMark := 0
        pacingStartTime := ""
        ...
        {
            PacingToggleBtn.Text := "BOT LOCK"
            PacingToggleBtn.SetFont("cFF00FF")
        }
        if IsObject(PacingProgressText)
        {
    ...
    ; Countdown pacing: current below target means the designated lot was passed.
...
RecalibratePacing()
{
    ...
    ; --- 2. CALCULATE "HOUR: LEFT" (Distance - Future Blocks) ---
    ...
    if (pacingStartTime != "" && pacingTargetAbsoluteLotMark > 0 && lotsToGo > 0 && secondsRemaining > 0)
    {
        ...
        if IsObject(PacingProgressText)
        {
            if (newWaitMs >= 60000)
            {
                PacingProgressText.SetFont("cFF66FF")
            ...
            {
                PacingProgressText.SetFont("cFF4DFF")
